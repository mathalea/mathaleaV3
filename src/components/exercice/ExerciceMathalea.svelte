<script lang="ts">
  ...
  let headerExerciceProps: { title: string; isInteractif: boolean; settingsReady?: boolean; isSortable?: boolean; isDeletable?: boolean; isHidable?: boolean } = {
            ...
            <div style="columns: {columnsCount.toString()}">
              <ul class="{exercice.listeQuestions.length > 1 ? 'list-decimal' : 'list-none'} list-inside mt-2 mx-2 lg:mx-6  marker:text-coopmaths marker:font-bold">
                {#each exercice.listeQuestions as item, i (i)}
                  <li style={i < exercice.listeQuestions.length ? `margin-bottom: ${exercice.spacing}` : ""} id="exercice${indiceExercice}Q${i}">
                    {@html item
                      .replace(/\dotfill/g, "..............................")
                      .replace(/\not=/g, "≠")
                      .replace(/\ldots/g, "....")}
                  </li>
                  <div class="bg-gray-200" style="line-height: {Math.max(3, exercice.spacingCorr * 2)}em;" id="correction${indiceExercice}Q${i}">
                    {@html exercice.listeCorrections[i]
                      .replace(/\dotfill/g, "..............................")
                      .replace(/\not=/g, "≠")
                      .replace(/\ldots/g, "....")}
                  </div>
