<script lang="ts">
  ...
  let isExercisesListVisible = true
  let filtre = "all"
    ...
    <main class="mb-auto flex flex-col md:flex-row h-full bg-coopmaths-canvas dark:bg-coopmathsdark-canvas text-coopmaths-corpus dark:text-coopmathsdark-corpus" on:mousemove={resizing}>
      ...
      {#if $exercicesParams.length !== 0}
        <div class="relative flex flex-col px-6 {deviceType() === 'mobile' ? '' : 'overflow-hidden'} w-full h-full" bind:this={divExercices}>
          ...
          <!-- barre des boutons -->
          <div class="flex flex-row justify-center space-x-20 items-center px-4 py-6">
            <!-- réglages pour tous les exercices de la page -->
            <div class="group inline-block relative">
              <button class="bg-coopmaths-canvas dark:bg-coopmathsdark-canvas text-coopmaths-action dark:text-coopmathsdark-action p-2 rounded-t-md">
              ...
              <div class="absolute inset-0 w-full group-hover:flex flex-row p-4 justify-center space-x-4 items-center hidden z-10 md:translate-x-20">
                <div class="grid grid-flow-col gap-2 place-content-center place-items-start bg-coopmaths-canvas-dark dark:bg-coopmaths-canvas-dark rounded-md  p-4">
                  ...
                  <button type="button" on:click={zoomPlus} class="tooltip tooltip-top tooltip-neutral" data-tip="Augmenter la taille du texte">
                    <i
                      class="bx {deviceType() === 'mobile'
                        ? 'bx-sm'
                  ...
                  >
                    <i
                      class="bx {deviceType() === 'mobile'
                        ? 'bx-sm'
                        : 'bx-md'} px-2 tooltip-top tooltip-neutral  hover:text-coopmaths-action-lightest text-coopmaths-action dark:text-coopmathsdark-action dark:hover:text-coopmathsdark-action-lightest {setAllInteractifClicked
                        ? 'bxs-edit'
                        : 'bx-edit'}"
                    />
                  </button>
                  <button type="button" on:click={newDataForAll} class="tooltip tooltip-top tooltip-neutral" data-tip="Nouveaux énoncés">
                    <i
                      class="bx {deviceType() === 'mobile'
                        ? 'bx-sm'
                  ...
                  {#if $globalOptions.v === "l"}
                    <div class="flex flex-row justify-end items-center">
                      <button type="button" on:click={quitFullScreen} class="tooltip tooltip-top tooltip-neutral" data-tip="Quitter le plein écran">
                        <i
            ...
            <div class="group inline-block relative">
              <button class="bg-coopmaths-canvas dark:bg-coopmathsdark-canvas text-coopmaths-action dark:text-coopmathsdark-action">
