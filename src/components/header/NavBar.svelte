...
<nav class="bg-coopmaths z-50">
  ...
  <div class="flex flex-wrap w-full mx-auto lg:space-x-6 lg:items-center">
    ...
    <div class="flex flex-row justify-start space-x-2">
      <button class=" lg:hidden inline-flex ml-auto items-center justify-center text-coopmaths-back font-bold text-xl" on:click={() => (isNavBarVisible = !isNavBarVisible)}>
        <i class="bx bx-menu" />
      </button>
      <!-- logo -->
      <a href={"#"} class="inline-flex p-2 text-2xl font-logo6 text-coopmaths-back uppercase tracking-wider">Coopmaths</a>
    </div>
    <!-- menu -->
    <div class="flex flex-col mt-2 lg:inline-flex lg:grow lg:flex-row lg:mt-0 {isNavBarVisible ? 'flex' : 'hidden'}">
      <ul class="flex flex-col space-y-2 lg:flex-row lg:space-y-0">
        ...
        <li>
          <a href={"https://coopmaths.fr/calculmental/"} class="bg-coopmaths hover:bg-coopmaths-light text-coopmaths-back text-xl font-extrabold relative block py-6 px-2 lg:p-6 items-center"
            >Calcul mental</a
          >
        </li>
        <li>
